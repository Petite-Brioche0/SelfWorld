...
class StaffPanelService {
	...
	async consumeImageMessage(message) {
		...
		try {
			...
			await message.reply('Impossible de récupérer cette image pour le moment.').catch(() => {});
			return true;
		}
	...
	async handleButton(interaction) {
		...
		if (!this.#isStaff(interaction)) {
			await this.#reply(interaction, { content: 'Accès réservé au staff.', flags: MessageFlags.Ephemeral });
			return true;
		}
		...
		if (id === 'staff:panel:refresh') {
			...
			await this.#followUp(interaction, { content: 'Panneau staff actualisé.', flags: MessageFlags.Ephemeral });
			return true;
		}
		...
		if (id.startsWith('staff:announce:edit:')) {
			...
			if (!announcement) {
				await this.#reply(interaction, { content: 'Annonce introuvable.', flags: MessageFlags.Ephemeral });
				return true;
			}
		...
		if (id.startsWith('staff:announce:send:')) {
			...
			const announcement = await this.#getAnnouncement(announcementId);
			if (!announcement) {
				await this.#followUp(interaction, { content: 'Annonce introuvable.', flags: MessageFlags.Ephemeral });
				return true;
			}
			try {
				await this.#sendAnnouncementNow(announcement, interaction.user.id);
				await this.#followUp(interaction, { content: 'Annonce envoyée.', flags: MessageFlags.Ephemeral });
			} catch (err) {
		...
		if (id.startsWith('staff:event:edit:')) {
			...
			if (!event) {
				await this.#reply(interaction, { content: 'Événement introuvable.', flags: MessageFlags.Ephemeral });
				return true;
			}
		...
		if (id.startsWith('staff:event:send:')) {
			...
			const event = await this.#getEventDraft(eventId);
			if (!event) {
				await this.#followUp(interaction, { content: 'Événement introuvable.', flags: MessageFlags.Ephemeral });
				return true;
			}
			try {
				await this.#activateEvent(event, interaction.user.id);
				await this.#followUp(interaction, { content: 'Événement envoyé.', flags: MessageFlags.Ephemeral });
			} catch (err) {
				this.logger?.warn({ err, eventId }, 'Failed to send event');
				await this.#followUp(interaction, {
					content: 'Impossible d’envoyer l’événement.',
					flags: MessageFlags.Ephemeral
				});
			}
			return true;
	...
	async #handleAnnouncementModal(interaction) {
		try {
			...
			if (pendingImage) {
				...
				await this.#reply(interaction, {
					content: 'Envoie l’image dans ce salon pour générer l’aperçu.',
					flags: MessageFlags.Ephemeral
				});
				return;
			}
			...
			await this.#reply(interaction, {
				content: 'Impossible de préparer l’annonce.',
				flags: MessageFlags.Ephemeral
			});
		}
	}
	...
	async #handleEventModal(interaction) {
		try {
			...
			if (!name) {
				await this.#reply(interaction, { content: 'Le nom est obligatoire.', flags: MessageFlags.Ephemeral });
				return;
			}
			...
			await this.#reply(interaction, {
				content: 'Impossible de préparer l’événement.',
				flags: MessageFlags.Ephemeral
			});
		}
	}
	...
	async #handleScheduleModal(interaction, kind) {
		try {
			...
			if (!recordId) {
				await this.#reply(interaction, { content: 'Identifiant invalide.', flags: MessageFlags.Ephemeral });
				return;
			}
			...
			await this.#reply(interaction, {
				content: `Programmé pour ${this.#formatSchedule(scheduledAt)}.`,
				flags: MessageFlags.Ephemeral
			});
		} catch (err) {
			this.logger?.warn({ err, kind }, 'Failed to schedule from modal');
			await this.#reply(interaction, {
				content: 'Impossible de programmer pour le moment.',
				flags: MessageFlags.Ephemeral
			});
		}
	}
	...
	async #scheduleEvent(id, scheduledAt, actorId) {
		...
		if (tempGroupService?.createGroup && event.guild_id) {
			...
			if (!group) {
				...
				await tempGroupService.updateExpiry(group.id, expiry).catch(() => {});
	...
	async #activateEvent(event, actorId) {
		...
		if (!group) {
			...
			await tempGroupService.updateExpiry(group.id, end).catch(() => {});
		}
