...
class ActivityService {
	...
	/** Post low-activity alerts directly in #reception (simple threshold example) */
	async postLowActivityAlerts() {
		const [zones] = await this.db.query('SELECT id, text_reception_id FROM zones');
		for (const z of zones) {
			const [rows] = await this.db.query(
				'SELECT SUM(msgs) AS s FROM zone_activity WHERE zone_id=? AND day >= DATE_SUB(CURRENT_DATE(), INTERVAL 14 DAY)',
				[z.id]
			);
			const s = rows?.[0]?.s || 0;
			if (s === 0) {
