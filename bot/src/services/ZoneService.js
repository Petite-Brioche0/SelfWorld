...
class ZoneService {
        ...
        async createZone(guild, { name, ownerUserId, policy }) {
                ...
                try {
                        ...
                        for (const channel of createdChannels) {
                                await channel.delete('Zone creation rollback').catch(() => {});
                        }
                        for (const role of createdRoles) {
                                await role.delete('Zone creation rollback').catch(() => {});
                        }
        ...
        async deleteChannel(channelId) {
                ...
                if (!channel || !channel.guild) throw new Error('Canal introuvable');
                const zone = await this.#getZoneByCategory(channel.parentId);
                if (!zone) throw new Error('Ce canal ne fait pas partie d’une zone gérée');
                await channel.delete('Zone channel delete').catch(() => {});
        ...
        async renameChannel(channelId, name) {
                ...
                const zone = await this.#getZoneByCategory(channel.parentId);
                if (!zone) throw new Error('Ce canal ne fait pas partie d’une zone gérée');
                await channel.setName(name.slice(0, 90), 'Zone channel rename').catch(() => {});
                await this.#refreshPanel(zone.id, ['channels']);
        }
        ...
        async deleteRole(zoneId, roleId) {
                ...
                if (!guild) throw new Error('Guilde introuvable');
                const role = await guild.roles.fetch(roleId).catch(() => null);
                if (role) await role.delete('Zone role delete').catch(() => {});
        ...
        async renameRole(zoneId, roleId, name) {
                ...
                await role.setName(name.slice(0, 100), 'Zone role rename').catch(() => {});
        ...
        async addMember(zoneId, userId) {
                ...
                if (!roleMember) throw new Error('Rôle membre manquant');
        ...
        async removeMember(zoneId, userId) {
                ...
                if (member) {
                        ...
                        if (roleOwner) await member.roles.remove(roleOwner).catch(() => {});
                }
