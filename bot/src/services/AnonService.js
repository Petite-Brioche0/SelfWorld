...
class AnonService {
	...
	async getTodayAnonMap({ channelId }) {
		if (!channelId) {
			return new Map();
		}

		const zoneId = await this.#findZoneByAnonChannel(channelId);
		if (!zoneId) {
			return new Map();
		}
		...
		const [rows] = await this.db.query(
				'SELECT DISTINCT author_id FROM anon_logs WHERE source_zone_id = ? AND created_at >= ? AND created_at < ? ORDER BY author_id',
				[zoneId, start, end]
		);

		const map = new Map();
		for (const row of rows || []) {
			const anonName = this.#buildAnonName(row.author_id, zoneId);
			map.set(anonName, String(row.author_id));
		}
